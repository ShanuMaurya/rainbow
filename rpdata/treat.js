...
const discards = require("./discards.json");
...
function gen_treat_regex() {
  ...
  var start_name_selector = "(?:[\\-=]\\> _(" + player_selector + ") )";
...
module.exports = function main(msg) {
  ...
  else if (msg.content.startsWith("xomi")) {
    console.log(msg.author.username + ":" + msg.author.discriminator + "  " + msg.content);
    ...
    if (commands[0] == "help") {
      msg.channel.send({embed: {
        ...
        fields: [
          ...
          {name: "Example", value: "`\*hug @shanu\*` Means that you hug the user **@sha_dryx**"},
